...
```sql
SELECT * 
  FROM LDAS0300('LD11', 引数.品目番号, 引数.供給者, 引数.使用者)
```
...
```sql
SELECT * 
  FROM LDAS0301( 'LD11', 引数.着手日, 引数.完了日, 引数.払出日, 引数.品目番号, 引数.供給者, 引数.使用者, 2.3.1で取得.MRP需要方針コード)
```
...
- (引数.入力元トランザクション = '18'  または '28' )かつ 2.3.1で取得.品目ステータス = '2'（生試品目）の場合
...
- 引数.入力元トランザクション = '28' かつ 2.3.1で取得.変動安全在庫数 ≠ 0 の場合、エラーメッセージを出力し処理終了。
  - エラーコード : E.LDP10510
...
2.3.6.1で取得.発注方針コード = 'S' の場合、納入基準日マスタをチェックする。
...
- 検索結果が0件でない かつ 引数.生試初品区分 = '3' かつ 2.3.1で取得.品目ステータス = '3' 場合、エラーメッセージを設定。
  - 処理ステータス: 1
  - エラーコード : E.LDP10473
...
- 検索結果が0件でない かつ 引数.生試初品区分 = '' かつ 2.3.1で取得.品目ステータス = '3' の場合、エラーメッセージを設定。
  - 処理ステータス : 1
  - エラーコード : E.LDP10475
...
| 26  | 納入日                     | I   | 引数.完了日                 |
| 27  | 払出日                     | I   | 引数.払出日                 |
| 28  | 完了開始時間               | I   | 引数.完了開始時間           |
| 29  | 完了終了時間               | I   | 引数.完了終了時間           |
| 30  | 繰越調整数                 | I   | 引数.繰越調整数             |
