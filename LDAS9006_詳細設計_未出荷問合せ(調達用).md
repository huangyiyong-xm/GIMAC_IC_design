...
| ------------ | ------------ | -------------------- |
| IC           | LDAS9006     | 未出荷問合せ(調達用) |

| RFC | Version | 更新日     | 更新者 | 更新内容 | 確認日     | 確認者 | 承認日     | 承認者 |
| --- | :-----: | ---------- | :----: | -------- | ---------- | :----: | ---------- | :----: |
| -   |  1.0.0  | 2025/09/21 | 李鵬陽 | 初版作成 | 2025/XX/XX |  XXX  | 2025/XX/XX |  XXX  |

## 1. 処理概要

### 1.1. 機能概要

データ出力ツール用パラメータ情報から、オーダ明細／独立所要量明細を取得し遅延日数を計算して戻り値で返す
...
suCodeGet["<div style='white-space: nowrap;'>2.2.3.抽出対象SUの取得</div>"]
ic_slip_date_get["<div style='white-space: nowrap;'>2.2.4.IC工場処理日取得</div>"]
...
| --- | ---------------- | ---------------- | ------- | --------------------------------------------- |
| 1   | 担当課           | ps_section       | 必須項目：使用者の担当課VARCHAR |                                               |
| 2   | 担当者           | ps_person_cd     | 使用者の担当者VARCHAR |                                               |
| 3   | 抽出期間開始日   | ps_start_date    | VARCHAR |                                               |
| 4   | 抽出期間終了日   | ps_end_date      | VARCHAR |                                               |
| 5   | 納入PF番号       | ps_deliv_pf_no   | VARCHAR |                                               |
| 6   | エリアカテゴリ   | ps_area_category | VARCHAR | 1:ALL2:社内3:社外 (新表替换为 エリアカテゴリ) |
| 7   | 使用者           | ps_usercd        | VARCHAR |                                               |
| 8   | 生試初品区分     | ps_pilot_class   | 必須項目：1:ALL2:量産3:初品4:生試／単品生試VARCHAR |                                               |
...
| 23  | MRP更新日        | rs_mrp_date         | VARCHAR |                                                    |
| 24  | 発注時着手日     | rs_rls_start_date   | VARCHAR |                                                    |
| 25  | OD/RDフラグ      | rs_flag             | VARCHAR |                                                    |
| 26  | 品目クラス       | rs_item_class       | VARCHAR |                                                    |
...
SELECT B.品目番号
 ...
 WHERE A.品目番号    = B.品目番号
   ...
   AND A.供給者     <> A.使用者
   AND C.SUコード    = A.供給者
   AND A.供給者     IN (SELECT su_code  FROM temp_date2)
   AND B.オーダーステータス  = '2'
   AND (引数.生試初品区分 = '1'
