...
| 6   | 社内／社外区分   | ps_org_category  | VARCHAR | 必須項目：1:ALL2:社内3:社外                 |
| 7   | 供給者           | ps_supplier      | VARCHAR |                                             |
| 8   | 生試区分         | ps_pilot_class   | VARCHAR | 必須項目：1:ALL2:量産3:初品4:生試／単品生試 |
| 9   | 先行リードタイム | ps_precede_lt    | INTEGER |                                             |
...
| No. | パラメータ論理名     | パラメータ物理名  | 属性    | 備考                                                 |
| --- | -------------------- | ----------------- | ------- | ---------------------------------------------------- |
| 1   | SPステータス         | rs_sp_status      | INTEGER | 0:Normal 100:Not Found -1:Sql Error -2:Program Error |
| 2   | SQLエラーコード      | rs_sql_error      | INTEGER |                                                      |
| 3   | ISAMエラーコード     | rs_isam_error     | INTEGER |                                                      |
| 4   | エラー位置           | rs_errs_focus     | VARCHAR |                                                      |
| 5   | プログラムステータス | rs_pg_status      | INTEGER |                                                      |
| 6   | 遅延日数             | rs_delay_days     | INTEGER |                                                      |
| 7   | 品目番号             | rs_itemno         | VARCHAR |                                                      |
| 8   | 品目ステータス       | rs_item_status    | VARCHAR |                                                      |
| 9   | 供給者               | rs_supplier       | VARCHAR |                                                      |
| 10  | 使用者               | rs_usercd         | VARCHAR |                                                      |
| 11  | 先行リードタイム     | rs_precede_lt     | INTEGER |                                                      |
...
| 17  | 払出日               | rs_disburse_time  | VARCHAR |                                                      |
| 18  | 所要数               | rn_required_qty   | INTEGER |                                                      |
| 19  | オーダー残数         | rn_bo_qty         | INTEGER |                                                      |
| 20  | 生試区分             | rs_code_exp       | VARCHAR |                                                      |
...
CREATE TEMP TABLE temp_date1(
     ...
     itemno                       CHAR(14),    --品目番号
     item_status                  CHAR(01),    --品目ステータス
     supplier                     CHAR(04),    --供給者
     usercd                       CHAR(04),    --使用者
     precede_lt                   INTEGER,     --先行リードタイム
     deliv_pf_no                  CHAR(04),    --納入PF番号
     stock_location               CHAR(03),    --格納ロケ
     item_name                    CHAR(30),    --品目名称
     total_orderno                CHAR(05),    --オーダ番号
     start_date                   CHAR(08),    --着手日
     disburse_time                CHAR(04),    --払出日
     required_qty                 INTEGER,     --所要数
     bo_qty                       INTEGER,     --オーダー残数
     code_exp                     CHAR(01),    --生試区分
     mrp_date                     CHAR(08),    --MRP更新日
     flag                         CHAR(01),    --OD/RDフラグ
     item_class                   CHAR(01),    --品目クラス
     measure_unit                 CHAR(01)     --測定単位
)ON COMMIT DROP;
```

```sql
CREATE TEMP TABLE temp_date2(
     su_code                      CHAR(04)     --SUコード
)ON COMMIT DROP;
```
...
```sql
INSERT INTO temp_date2 (su_code)
SELECT エリアコード
  FROM GIMACエリアマスタ
 WHERE エリアカテゴリ IN ('06','56')
   AND 担当課 = 引数.担当課
   AND (引数.担当者 = ' ' or 担当者 = 引数.担当者)
```
...
SELECT B.品目番号,  
 ...
 WHERE A.品目番号    = B.品目番号
   ...
   AND B.オーダー残数 > 0
   AND (引数.生試初品区分 = '1'
       OR (引数.生試初品区分 = '2' AND B.生試初品区分 = ' ')
       OR (引数.生試初品区分 = '3' AND B.生試初品区分 = '3')
       OR (引数.生試初品区分 = '4' AND B.生試初品区分 IN ('2','4')))
...
SELECT D.品目番号
 ...
 WHERE A.供給者     <> A.使用者
   ...
   AND D.オーダー残数 > 0
   AND (引数.生試初品区分 = '1'
       OR (引数.生試初品区分 = '2' AND D.生試初品区分 = ' ')
       OR (引数.生試初品区分 = '3' AND D.生試初品区分 = '3')
       OR (引数.生試初品区分 = '4' AND D.生試初品区分 IN ('2','4')))
   AND C.SUコード      = A.使用者
   AND ((A.Wビン管理コード = '0' AND 引数.Ｗビン管理コード = '0')
       OR 引数.Ｗビン管理コード = '1')
   AND (引数.社内／社外区分 = '1'
