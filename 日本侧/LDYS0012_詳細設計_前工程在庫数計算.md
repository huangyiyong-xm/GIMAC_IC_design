...
- データが存在しない場合
  ...
  - 引数.供給者 = 引数.使用者 の場合     ※1レベル展開
    ...
    - それ以外の場合
      製品構成を検索する
      ```sql
          SELECT comp_itemno,                      --子品目番号
                 comp_supplier,                    --子供給者
                 comp_usercd,                      --子使用者
            INTO ls_itemno,                        --変数.品目番号
                 ls_supplier,                      --変数.供給者
                 ls_usercd                         --変数.使用者
            FROM la_prodstrc                       --製品構成 
           WHERE parent_itemno     = ps_itemno
...
```sql
  IF EXISTS (SELECT 1
   ...
   SELECT airs_sign                 --AIRSサイン
     INTO ls_airs_sign              --変数.AIRSサイン
     FROM le_mst_mrp_information    --MRP情報値 
    WHERE itemno   = ps_itemno             
      AND supplier = ps_supplier              
      AND usercd   = ps_usercd 
...
- 変数.AIRSサイン <> '0'の場合 ループ継続
  ...
  ```sql
   SELECT COUNT(1)
     FROM la_prodstrc                        --製品構成
    WHERE parent_itemno      =  ls_itemno    --変数.品目番号
      AND  parent_supplier   =  ls_supplier  --変数.供給者
      AND  parent_usercd     =  ls_usercd    --変数.使用者
      AND  in_effective_ymd <=  ps_date
      AND  out_effective_ymd >  ps_date
  ```
...
```sql
  IF EXISTS (SELECT 1
              ...
              WHERE itemno   = ls_itemno       --変数.品目番号
                AND supplier = ls_supplier     --変数.供給者
                AND usercd   = ls_usercd       --変数.使用者
  )  THEN
   SELECT oh_qty                     --手持在庫数
     INTO ls_oh_qty                  --変数.手持在庫数
     FROM ld_trn_inv                 --在庫ファイル 
    WHERE itemno   = ls_itemno       --変数.品目番号
      AND supplier = ls_supplier     --変数.供給者
      AND usercd   = ls_usercd       --変数.使用者
