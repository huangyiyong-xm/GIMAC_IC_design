...
flowchart LR
   ...
   subgraph mainBlock ["2.3.主処理"]
     ...
     exisitCheck --> loopCheck["<div style='white-space: nowrap;'>2.3.5.3.無限ループ回避チェック</div>"]
...
UPDATE ld_trn_inv                                          --在庫ファイル
   SET ic_update_datetime = statement_timestamp()          --IC更新日時
      ,update_author = ps_user_id                          --更新者
      ,update_pgmid = 'LDYS0003'                            --更新pgid
...
```sql
SELECT next_ohorder_ic         --接頭番号（ｵﾝﾗｲﾝ）
      ,next_odorder_ic         --順序番号（ｵﾝﾗｲﾝ）
      ,next_bhorder_ic         --接頭番号（ﾊﾞｯﾁ）
      ,next_bdorder_ic         --順序番号（ﾊﾞｯﾁ）
    INTO ls_next_ohorder_ic    --変数.接頭番号（ｵﾝﾗｲﾝ）
         ls_next_odorder_ic    --変数.順序番号（ｵﾝﾗｲﾝ）
         ls_next_bhorder_ic    --変数.接頭番号（ﾊﾞｯﾁ）
         ls_next_bdorder_ic    --変数.順序番号（ﾊﾞｯﾁ）
  FROM ld_trn_inv              --在庫ファイル 
 WHERE itemno = ps_itemno
   AND supplier = ps_supplier
   AND usercd = ps_usercd;
...
IF NOT EXISTS (SELECT 1
                ...
                WHERE order_no = ls_order_no        --変数.オーダNo.
...
```sql
UPDATE ld_trn_inv                                         --在庫ファイル
   SET next_ohorder_ic = ls_next_ohorder_ic               --変数.接頭番号（ｵﾝﾗｲﾝ）
      ,next_odorder_ic = ls_next_odorder_ic               --変数.順序番号（ｵﾝﾗｲﾝ）
      ,update_counter = COALESCE(update_counter, 0) + 1  --更新カウンター
      ,ic_update_datetime = statement_timestamp()         --更新日時
      ,update_author = ps_user_id                         --更新者
      ,update_pgmid = 'LDYS0003'                           --更新pgid
 WHERE itemno = ps_itemno
   AND supplier = ps_supplier
...
```sql
UPDATE ld_trn_inv                                                --在庫ファイル 
   SET next_bhorder_ic = ls_next_bhorder_ic                      --変数.接頭番号（ﾊﾞｯﾁ）
      ,next_bdorder_ic = ls_next_bdorder_ic                      --変数.順序番号（ﾊﾞｯﾁ）
      ,update_counter = COALESCE(update_counter, 0) + 1         --更新カウンター
      ,ic_update_datetime = statement_timestamp()                --更新日時
      ,update_author = ps_user_id                                --更新者
      ,update_pgmid = 'LDYS0003'                                  --更新pgid
