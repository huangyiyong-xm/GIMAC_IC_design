...
| 11  | 担当課                 | rs_org_section        | VARCHAR |                                      |
| 12  | 担当者                 | rs_org_person_cd      | VARCHAR |                                      |
| 13  | 工程番号               | rs_process_no         | VARCHAR |                                      |
| 14  | 投入作業区             | rs_input_work_center  | VARCHAR |                                      |
| 15  | 納入PF番号             | rs_deliv_pf_no        | VARCHAR |                                      |
| 16  | 出庫数－SIR            | rn_toq_sir            | VARCHAR |                                      |
| 17  | 出庫数－HAND           | rn_toq_hand           | DECIMAL |                                      |
| 18  | 出庫数－非製造         | rn_toq_nonmanufact    | DECIMAL |                                      |
| 19  | 出庫数－仕損（使用中） | rn_toq_scrap_use      | DECIMAL |                                      |
| 20  | 出庫数－仕損（SIR）    | rn_toq_scrap_sir      | DECIMAL |                                      |
| 21  | 出庫数－サービス       | rn_toq_service        | DECIMAL |                                      |
| 22  | 出庫数－CKD            | rn_toq_ckd            | DECIMAL |                                      |
| 23  | 出庫数－返品           | rn_toq_back           | DECIMAL |                                      |
| 24  | 製造中仕損率           | rn_manuf_scrap_ratio  | DECIMAL |                                      |
| 25  | 使用中仕損率           | rn_use_scrap_ratio    | DECIMAL |                                      |
| 26  | 入庫数（累計）         | rn_total_in_qty       | DECIMAL |                                      |
| 27  | 出庫数－仕損（製造中） | rn_toq_scrap_manufact | DECIMAL |                                      |
| 28  | 品目クラス             | rs_item_class         | VARCHAR |                                      |
| 29  | 品目分類コード         | rs_reference_key      | VARCHAR |                                      |
| 30  | エリア略称             | rs_org_abbr           | VARCHAR |                                      |
...
| 3  | テーブル   | 棚卸管理マスタ         | ld_mst_st_control      | - | ○ | - | - | (旧)ld_jp_st_cntl_mast |
| 4  | テーブル   | 棚卸前理論在庫ファイル | ld_trn_st_logic_inv    | - | ○ | - | - | (旧)ld_jp_st_logic_inv |
