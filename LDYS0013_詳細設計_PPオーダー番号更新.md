...
| 2   | 供給者               | ps_supplier        |      |      |
| 3   | 使用者               | ps_usercd          |      |      |
| 4   | 接頭番号(オンライン) | ps_next_ohorder_ic |      |      |
| 5   | 順序番号(オンライン) | ps_next_odorder_ic |      |      |

#### 1.3.2. 戻り値

| No. | パラメータ論理名 | パラメータ物理名 | 属性 | 備考                      |
| --- | ---------------- | ---------------- | ---- | ------------------------- |
| 1   | 処理ステータス   | rn_status        |      | 0:正常終了,-1,-2:異常終了 |
...
```sql
UPDATE 在庫ファイル a
   SET a.接頭番号（オンライン） = 引数.接頭番号（オンライン）
      ,a.順序番号（オンライン） = 引数.順序番号（オンライン）
      ,a.更新カウンター = a.更新カウンター + 1
      ,a.更新日時 = システム時間
      ,a.更新者 = 'SYSTEM'
      ,a.更新pgid = 'LDYS0013'
 WHERE a.品目番号 = 引数.品目番号
