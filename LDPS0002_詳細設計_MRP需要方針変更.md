...
品目マスタのMRP需要方針コードが変更された時、MRP対象からハンド管理に変更された時、内示（オーダーステータス＝０，１）オーダを削除して、従属所要量も削除する。
※
オーダー明細が下記条件の①～④のいずれかを満たす場合は、なにもしない。
①
オーダー明細.オーダーステータスが‘０’、‘１’以外である。
②
...
```
    ...
    ループ START
        ...
        CALL SP LDAS0414 オーダー明細削除    確認待ち(LDAS0414 还未作成)
...
```
        変数.処理ステータス <> 0  の場合、エラーメッセージを出力し処理終了
            - エラーコード：'E.XXXXX'  確認待ち
            - エラーメッセージ：'le_trn_order_forecast' || '<<SP ld0sl4140 Error Return>>' || 'Return:     変数.処理ステータス,変数.SQLコード,変数.エラーコード,変数.エラーメッセージ,変数.エラー位置' -->
