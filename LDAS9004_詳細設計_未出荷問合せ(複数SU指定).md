...
| 16  | 払出日           | rs_disburse_time    | VARCHAR |                                                    |
| 17  | 所要数           | rn_required_qty     | DECIMAL |                                                    |
| 18  | オーダー残数     | rn_bo_qty           | DECIMAL |                                                    |
| 19  | 前工程在庫数     | rn_b_prj_stock_qty  | DECIMAL |                                                    |
| 20  | 引当残数         | rn_alloc_remain_qty | DECIMAL |                                                    |
| 21  | 生試初品区分     | rs_pilot_class      | VARCHAR |                                                    |
| 22  | MRP更新日        | rs_mrp_date         | VARCHAR |                                                    |
| 23  | 発注時着手日     | rs_rls_start_date   | VARCHAR |                                                    |
| 24  | OD/RDフラグ      | rs_flag             | VARCHAR |                                                    |
| 25  | 品目クラス       | rs_item_class       | VARCHAR |                                                    |
...
CREATE TEMP TABLE temp_date(
     ...
     mrp_date                     VARCHAR(08), --MRP更新日
     rls_start_date               VARCHAR(08), --発注時着手日
     od_rd_flag                   VARCHAR(01), --OD/RDフラグ  
     item_class                   VARCHAR(01)  --品目クラス
) ON COMMIT DROP;
```
