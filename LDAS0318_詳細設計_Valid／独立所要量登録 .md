...
```sql
SELECT 有効独立需要送り先コード区分, 送り先/振替先等価区分
  FROM 独立需要送り先/費用振替先チェックテーブル
 WHERE 対象組織コード = 引数.独立需要送り先コード;
```

- 有効独立需要送り先コード区分 = 'Y'の場合：有効な送り先として処理継続
- 送り先/振替先等価区分 = 'Y'の場 合：独立需要送り先コードと費用振替先コードが同一である必要がある
  - 異なる場合のエラー：
    - エラーコード：'E.LDP10512'
...
```sql
SELECT * 
  FROM LEYS0001(2.3.2.1で取得.現在有効カレンダー, 引数.着手日, 2.3.7.1で取得.削除登録可能日数)
```
