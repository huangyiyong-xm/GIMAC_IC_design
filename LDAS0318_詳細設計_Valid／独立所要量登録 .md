...
```sql
SELECT * 
  FROM LDAS0300('LD21', 引数.品目番号, 引数.供給者, 引数.使用者)
```
...
- 引数.独立需要送り先区分が空白または引数.独立需要送り先コードが空白の場合
  - エラーコード：'E.LDP10428'
...
###### 2.3.5.4.1. 独立需要送り先振替チェックマスタ確認（確認待ち）

```sql
SELECT valid_ind_user_yn, ind_user_eq_transfer_yn
  FROM lc_mst_ind_user_transfer_check
 WHERE target_org_code = 独立需要送り先コード;
```
...
- valid_ind_user_yn = 'Y'の場合：有効な送り先として処理継続
- ind_user_eq_transfer_yn = 'Y'の場 合：独立需要送り先コードと費用振替先コードが同一である必要がある
  - 異なる場合のエラー：
    - エラーコード：'E.LDP10512'
...
稼働日非チェックフラグが'Y'でない場合、以下チェックを実行
...
```sql
SELECT 独立所要量登録可能日数, 先行RD削除日登録可能日数
  FROM MRPシステムパラメーター;
```
...
```sql
SELECT * 
  FROM LEYS0001(2.3.2.1で取得.現在有効カレンダー, IC工場処理日, 2.3.7.1で取得.独立所要量登録可能日数)
```
...
```sql
SELECT * 
  FROM LEYS0001(2.3.2.1で取得.現在有効カレンダー, 着手日, 2.3.7.1で取得.削除登録可能日数)
```
...
2.3.12.1で取得.エリアカテゴリが'56'（Ｓ/Ｕ（社外））の場合：
